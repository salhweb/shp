...
/**
	...
	//	$users = $query->paginate(50)->appends($queryStr);		 simplePaginate
	    ...
	    $user_roles=DB::select(DB::raw($stSql));
 	         return View::make('admin/users.index', array('users' => $users , 'user_roles'=>$user_roles));
	...
	public function store()
	{
		...
		{	
    			...
    			['student_id' => $userid, 'school_id' => Input::get('school'), 'birth_date' => Input::get('birth_date'),'class_id' => Input::get('class_id'),'section_id' => Input::get('section_id'),'gender' => Input::get('gender'),'father_name' => Input::get('father_name'),'mother_name' => Input::get('mother_name'),'phone_no' => Input::get('phone_no'),'address' => Input::get('address')]
			);
	...
	public function idcard()
	{
	 ...
	 $query = DB::table('users')->select('users.*','student_details.*','classes.class_name')->join('student_details','student_details.student_id','=','users.id')->join('classes','classes.id','=','student_details.class_id');
	 	if(Input::get('school_id') !=""){
			$query = $query->where('student_details.school_id', '=', Input::get('school_id'));
...
<div class="card '.$card_class. '">
	<div class="header">
		<h1>Lala Jagan Nath Jain Public School</h1>
		<h3>Dada Bari, Rania Road, Sirsa-125055 (Haryana) 01666-240220</h3>
	</div>
...
<img src="'.Config::get('app.SITE_URL').'/storage/user_pics/lala_Jagan_Nath_principal_sign.jpg" style="height:20px;  margin: 0; width: 80px; float: left;padding-left:10px;" alt="">
...
<td>
<img style="height:125px; width: 125px; margin: 0;  border: 3px solid #ffe1be;"  src="data:image/png;base64, '.base64_encode(QrCode::format("png")->size(150)->

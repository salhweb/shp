	...
	@else
		@if(Auth::user())
			@if(Auth::user()->hasRole('administrator'))
				...
				@foreach($user_basic_detail as $user_detail)
				@if($user_detail->user_role==1)
				@elseif($user_detail->user_role==2)
				<div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
              <span class="count_top"><i class="fa fa-user"></i> Total Nurses</span>
              <div class="count">{{$user_detail->total}}</div>
              <a class="pull-right" href="{{url('nurses')}}">View</a>
            </div>
				@elseif($user_detail->user_role==3)
				<div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
              <span class="count_top"><i class="fa fa-user"></i> Total Doctors</span>
              <div class="count">{{$user_detail->total}}</div>
              <a class="pull-right" href="{{url('doctors')}}">View</a>

            </div>
				@elseif($user_detail->user_role==4)
				<div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
              <span class="count_top"><i class="fa fa-user"></i> Total Schools</span>
              <div class="count">{{$user_detail->total}}</div>
              <a class="pull-right" href="{{url('schools')}}">View</a>

            </div>
				@elseif($user_detail->user_role==5)
				<div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
              <span class="count_top"><i class="fa fa-user"></i> Total Students</span>
              <div class="count">{{$user_detail->total}}</div>
              <a class="pull-right" href="{{url('students')}}">View</a>
...
<?php if($user_detail[0]->birth_date!='0000-00-00')
   ...
   <?php if(count($user_basic_detail)>0)
		{ ?>
        <tr>
		<td>Blood Group</td><td style="text-transform: capitalize;"><?php echo $user_basic_detail[0]->b_grp; ?></td>  
           </tr>
           <tr>
           <td>Height</td><td><?php echo $user_basic_detail[0]->height; ?> Cm</td>  
           </tr>
           <tr>
           <td>Weight</td><td><?php echo $user_basic_detail[0]->weight; ?> Kg</td>  
           </tr>
           <?php } ?>
           </table>
            <div class="progress">
...
<div class="col-md-3">
        ...
        <tr>
        <td>Allergies 2:</td><td> <?php echo $user_basic_detail[0]->m_s_allergies_2; ?></td>
        </tr>
        <tr>
        <td>Allergies 3:</td><td> <?php echo $user_basic_detail[0]->m_s_allergies_3; ?></td>
